            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # --- Correctness-gated distillation reward clamp (OPD variant) ---
                        # Applies to the distillation reward (token_level_rewards) for BOTH:
                        #   top_k=0 : sampled-token OPD, reward = -(log p_s - log p_t) on the SAMPLED tokens (weight 1)
                        #   top_k>0 : top-k OPD,         reward = -kl_val * norm_weights on the top-k tokens (w >= 0)
                        # correctness_gated_mode="default":
                        #   correct -> keep only rm_scores >= 0  (equiv. kl_val <= 0); wrong -> keep only <= 0 (kl_val >= 0)
                        # correctness_gated_mode="inverse" (ablation, exactly complementary zeroed set):
                        #   correct -> keep only rm_scores <= 0  (equiv. kl_val >= 0); wrong -> keep only >= 0 (kl_val <= 0)
                        # Exact equivalence: rm_scores = -kl_val * norm_weights with norm_weights >= 0
                        # (norm_weights = 1 for top_k=0; softmax-over-K >= 0 for top_k>0). Guard on self.use_rm so we
                        ...
                        if (rollout_cfg.get("correctness_gated", False)
                            ...
                            thr = rollout_cfg.get("correctness_threshold", 0.0)
                            gated_mode = rollout_cfg.get("correctness_gated_mode", "default")
                            tlr = batch.batch["token_level_rewards"]                       # (B,T) or (B,T,K)
                            correct = (batch.batch["true_reward_score"].sum(dim=-1) > thr)  # bool (B,)
                            correct_b = correct.view([-1] + [1] * (tlr.dim() - 1))          # broadcast to tlr
                            if gated_mode == "inverse":
                                clamped = torch.where(correct_b, tlr.clamp(max=0.0), tlr.clamp(min=0.0))
                            else:
                                clamped = torch.where(correct_b, tlr.clamp(min=0.0), tlr.clamp(max=0.0))
                            batch.batch["token_level_rewards"] = clamped
                            if "token_level_scores" in batch.batch.keys():
                                batch.batch["token_level_scores"] = clamped  # keep reward stats consistent
                            n_resp = correct.numel()
                            metrics["cg/correct_frac"] = float(correct.sum().item()) / max(n_resp, 1)
                            denom = int((tlr != 0).sum().item())
                            metrics["cg/reward_zeroed_frac"] = int(((tlr != 0) & (clamped == 0)).sum().item()) / max(denom, 1)
                            print(f"[correctness_gated:{gated_mode}] correct {int(correct.sum().item())}/{n_resp}, "
                                  f"reward_zeroed_frac={metrics['cg/reward_zeroed_frac']:.3f}")

                        # --- GRPO group-relative correctness scaling (OPD variant) ---
                        # Scale the distillation reward by |GRPO outcome advantage| computed from per-response
                        # correctness (true_reward_score), grouped by uid. Reuses core_algos.compute_grpo_outcome_advantage.
                        #   grpo_norm_by_std=True : standard GRPO, a_i = (c_i - mu_g) / (sigma_g + eps)
                        #   grpo_norm_by_std=False: Dr.GRPO,       a_i = c_i - mu_g   (no /std)
                        # scale = |a_i| + u  (u = grpo_scale_baseline). Uniform groups (all-correct/all-wrong) have
                        # a_i = 0, so with u > 0 they still learn with distillation intensity u (teacher signal is
                        # kept) instead of being zeroed; mixed groups get |a_i| + u.
                        # Applied AFTER the gated clamp so it composes: reward = clamp(...) * (|a_i| + u).
                        if (rollout_cfg.get("grpo_scaled", False)
                                and self.use_rm
                                and "token_level_rewards" in batch.batch.keys()
                                and "true_reward_score" in batch.batch.keys()
                                and "uid" in batch.non_tensor_batch.keys()):
                            tlr = batch.batch["token_level_rewards"]
                            baseline_u = float(rollout_cfg.get("grpo_scale_baseline", 0.0))
                            grpo_adv, _ = core_algos.compute_grpo_outcome_advantage(
                                token_level_rewards=batch.batch["true_reward_score"],
                                response_mask=batch.batch["response_mask"],
                                index=batch.non_tensor_batch["uid"],
                                epsilon=1e-6,
                                norm_adv_by_std_in_grpo=rollout_cfg.get("grpo_norm_by_std", True),
                            )  # (B, T): per-response advantage broadcast over valid tokens, 0 on padding
                            scale = grpo_adv.abs().max(dim=-1).values + baseline_u  # (B,) = |a_i| + u
                            scale_b = scale.to(tlr.dtype).view([-1] + [1] * (tlr.dim() - 1))
                            scaled = tlr * scale_b
                            batch.batch["token_level_rewards"] = scaled
                            if "token_level_scores" in batch.batch.keys():
                                batch.batch["token_level_scores"] = scaled  # keep reward stats consistent
                            metrics["gs/scale_mean"] = float(scale.mean().item())
                            metrics["gs/scale_min"] = float(scale.min().item())
                            metrics["gs/zeroed_resp_frac"] = float((scale == 0).float().mean().item())
                            print(f"[grpo_scaled] u={baseline_u}, scale_mean={metrics['gs/scale_mean']:.3f}, "
                                  f"scale_min={metrics['gs/scale_min']:.3f}, "
                                  f"zeroed_resp_frac={metrics['gs/zeroed_resp_frac']:.3f}")
